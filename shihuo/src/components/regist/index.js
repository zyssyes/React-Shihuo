...
　}
  saveUser(props) {
    ...
    if(!psw) return alert('密码不能为空');
     axios.post("http://localhost:3001/api/regist", {
      ...
      <div className="Regist" id='Regist'>
        ...
        <div className='home-wrap' onChange={(e)=>this.stateChange(e)}>
          ...
          <div className='password'>
            ...
            <Link to="/login"><input  type="button" value="登录" className="login-btn" dace-node="8000_login" /></Link>
            <input   type="button" value="注册" className="regist-btn" dace-node="8000_login" onClick={()=>this.saveUser()} />
        </div>
